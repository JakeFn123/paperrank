...
4. **Skill 机检契约（JSON Schema）**  
- 每个 skill 增加 `CONTRACT.json`（输入/输出 schema）。  
- 在 Agent Loop 每个阶段执行前后自动校验，异常直接中断并报错。  

5. **结论-证据对齐表（新增报告段落）**  
- 综合报告自动追加 `## 结论-证据对齐表`，按“结论句 -> 引用 -> 证据片段”对齐输出。
...
| Skill | 强化点 |
|---|---|
| `query-decomposition` | 增加 `intent_slots` 覆盖、质量门禁、相关性自检、失败回退 |
| `academic-retrieval` | 增加两阶段检索、0命中简化重试、`__postprocess__` 日志审计 |
| `evidence-grading` | 增加证据兜底、总分可复算约束、评分可解释性门禁 |
| `synthesis` | 增加 `evidence_audit` 审查流程与未通过时自动追加审查说明 |

### 6.4 Skill 编写标准（V2.2）

为减少技能漂移，4 个技能统一采用以下结构：
...
| `app/agentic/skills.py` | 支持加载 `CONTRACT.json` 并做输入/输出 schema 校验 | 合同字段不匹配时抛出校验错误 |
| `skills/*/CONTRACT.json` | 每个技能新增可机检输入输出契约 | `SkillRegistry` 可成功加载 4 份合同 |
